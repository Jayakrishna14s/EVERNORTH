...
const OtpVerification = () => {
    ...
    const handleOtpSubmit = async () => {
        try {
            const response = await axios.post(`http://localhost:8081/api/auth/verify-otp`, { membershipId, otp });
            if (response.status === 200) {
                setMessage('OTP verified successfully! Proceed with profile setup.');

                // Store membershipId in localStorage upon successful OTP verification
